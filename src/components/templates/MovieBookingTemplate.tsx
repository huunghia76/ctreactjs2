...
export const MovieBookingTemplate = () => {
   ...
   const calculateTotalPrice = (): number => {
      return infoBooking.reduce((total, item) => total + item.price, 0);
   };
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="">
                  ...
                  <div className="grid grid-cols-12 gap-3">
                     {boxOfficeList?.danhSachGhe?.map((ghe: Ghe) => (
                        <Chair
                           key={ghe.maGhe}
                           className={cn("Chair border border-dark rounded d-flex mt-3", {
               ...
               <div className="!ml-18">
                  ...
                  {infoBooking.length ? (
                     <Button
                        ...
                        type="primary"
                        onClick={() => {
                           const listBooking = boxOfficeList?.danhSachGhe?.filter((e: Ghe) =>
                              booking.includes(e.tenGhe)
                           ) || [];
                           dispatch(
                              datVeThunk({
                                 maLichChieu: Number(boxOfficeList?.thongTinPhim?.maLichChieu),
