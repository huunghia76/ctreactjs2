...
import { Cinema, HeThongRapChieu, LichChieu, Rap } from "types";
...
function Detail() {
   ...
   const [postCinema, setPostCinema] = useState<HeThongRapChieu[]>([]);
   //nen tao type cho nó nha
   ...
   const fetchPostCinema = async () => {
      try {
         ...
         const dataPost: HeThongRapChieu[] = data.data.content?.heThongRapChieu;
   ...
   const handleShowCinema = (value: Cinema, index: number) => {
      return (
         <div
            key={index}
            style={{
   ...
   const handleShowData = () => {
      return postCinema?.map((value: Cinema, index: number) => {
         if (value?.maHeThongRap !== clickCinema) {
            return handleShowCinema(value, index);
         }
   ...
   const handleShowCinemaMovie = () => {
      return dataCinimaMovie?.map((e: Rap, index: number) => {
         const lichChieu = e?.lichChieuPhim;

         return (
            <Row gutter={[16, 16]} style={{ marginBottom: "30px" }} key={index}>
               <Col span={3}>
                  <div style={{ width: "50px", height: "50px" }}>
                     <img
                        src={e?.hinhAnh}
               ...
               <Col span={21}>
                  ...
                  <div style={{ display: "flex", marginTop: "5px" }}>
                     {lichChieu?.map((item: LichChieu, index: number) => (
                        <p
                           key={index}
                           style={{
                              marginRight: "10px",
